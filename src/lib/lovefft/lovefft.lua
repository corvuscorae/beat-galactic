...
local loveFFT = {}
loveFFT.__index = loveFFT

function loveFFT:new(fftSize, channelID)
    local instance = setmetatable({}, loveFFT)

    instance.channelID = channelID or tostring(math.random())
    instance:init(fftSize)
...
function loveFFT:push() -- Launch a new FFT computation in a separate thread using self.playPosition. Set the position using self.updatePlayTime or self.setPlayPosition
    ...
    for i = 1, self.fftSize do
        ...
        for j = 1, self.channelCount do
            ...
            local sampleIndex = sample + i - 1
            local totalSamples = self.soundData:getSampleCount()

            -- make sure we're within bounds
            if sampleIndex >= 0 and sampleIndex < totalSamples then
                toFFT[i] = toFFT[i] + self.soundData:getSample(sampleIndex, j)
            end
        end
    end
    -- Send to thread
...
function loveFFT:release() -- Releases the thread and clears the channel. According to LOVE documentation, this is necessary on Android platforms. But based on my personal experience, it is always unnecessary to call this function
    local stopChannel = love.thread.getChannel("stop_" .. self.channelID)
